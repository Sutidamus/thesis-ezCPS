...
(define-class let-named-exp Expression
    (constructor (lambda (self proc-id ids init-vals bodies)
                    (cond 
                        [(not (symbol? proc-id)) (. console log "Failed to create let-named-exp:: proc-id not a symbol")]
        ...
        [(pair? datum) 
            (let ([pair-type-id (car datum)])
                (cond
                    ...
                    [(equal? pair-type-id 'let)
                        (let ([var-pairs (2nd datum)]
                              [bodies (cddr datum)])
                              (if (symbol? (2nd datum))
                                (new let-named-exp 
                                  (parse-exp (2nd datum)) 
                                  (map parse-exp (map 1st (3rd datum)))
        ...
        (let helper ((exp (syntax-expand (parse-exp raw-code)))
            ...
            (cond 
                [(app-exp? exp) 
                    ...
                    (helper exp.rator #f)
                    (if (not in-tail-pos) 
                        (if (not (member exp.rator.id prim-procs))
                            (console.log "Not in tail postion: " exp.rator)
                        )
                    )
                ]
                [(if-exp? exp)
